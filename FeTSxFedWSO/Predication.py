...
# =========================
# Matrix Figure (legend bottom-center, aligned under the grid)
# =========================
def make_matrix_figure(configs: List[Dict], dataset, sample_index: int,
    ...
    # Slice selection via reference model
    ...
    # Leave bottom space for the legend (centered)
    ...
    for i, S in enumerate(S_vals):
        for j in range(ncols):
            ...
            ax.imshow(compose_gt_pred(frame_rgb, gt2d, entry["pred"], a_pr))
            if i == 0:
    ...
    # ===== Title positioned between C=3 and C=32 =====
    fig.canvas.draw()
    c3_box = axes[0, 1].get_position(fig)
    c32_box = axes[0, 2].get_position(fig)
    x_mid_c3_c32 = 0.5 * (c3_box.x0 + c32_box.x1)
    fig.suptitle(regime_title, fontsize=14, fontweight="bold", y=0.97, x=x_mid_c3_c32)

    # ===== Legend bottom-center, aligned under the GRID (not whole page) =====
    # Use full grid extent in figure coords
    boxes = [ax.get_position(fig) for ax in axes.ravel()]
    xmin = min(b.x0 for b in boxes)
    xmax = max(b.x1 for b in boxes)
    x_center_grid = 0.5 * (xmin + xmax)
    ...
    fig.legend(
        ...
        loc="lower center",
        bbox_to_anchor=(x_center_grid, 0.06),
